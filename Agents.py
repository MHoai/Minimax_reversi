...
def alphaBeta(cur_state, player_to_move, alpha, beta, remain_time, depth):
    boards = []
    choices = []
    ...
    if player_to_move == 1:
        ...
        bestChoice = []
        for board in boards:
            boardValue = alphaBeta(board, -player_to_move, alpha, beta, remain_time, depth - 1)[0]
            if boardValue>v:
                v = boardValue
                bestBoard = board
...
def select_move(cur_state, player_to_move, remain_time):
    result = alphaBeta(cur_state, player_to_move, -float("inf"), float("inf"), remain_time, DEPTH)
